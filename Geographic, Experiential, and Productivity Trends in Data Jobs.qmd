...
ggplot(jobs_summary1, aes(y = job_category, x = count, fill = work_setting)) +
  geom_col(position = "fill") +  # Use "fill" for proportional stacking
...
ggplot(productivity_data, aes(x = Productivity_Score, y = Hours_Worked_Per_Week, color = Employment_Type)) +
  ...
  scale_color_viridis_d(option = "D") +  # Viridis discrete palette
  ...
  theme(
    ...
    legend.position = "bottom"  # Place legend at the bottom for better aesthetics
  )
```
